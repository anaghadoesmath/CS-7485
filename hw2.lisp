...
;; helper predicate for rationals
(definec rat-okp (x :rat-err) :bool
	 (rationalp x))

(definec rat-nonzerop (x :rat-err) :bool
	 (and (rationalp x) (not (equal x 0))))

;; semantic evaluator for sael expressions
              ...
              (let ((vy (saeval y a)))
                (if (rationalp vy)
                    (cond
                      ...
                      ((equal op '-) (- vx vy))
                      ((equal op '*) (* vx vy))
                      ((equal op '/) (if (equal vy 0) *er* (/ vx vy)))
                      ((equal op '^)
                       (cond
                         ((not (integerp vy)) *er*)
                         ((and (equal vx 0) (< vy 0)) *er*)
                         (t (expt vx vy))))
                      (t *er*))
                    vy))
              vx)))))))
  ...
  (match e
    ...
    (:bsaexpr
     (match e
       ((x op y)
        (cond
          ((equal op '+) (list '+ (sael->aa x) (sael->aa y)))
          ((equal op '-) (list '- (sael->aa x) (sael->aa y)))
          ((equal op '*) (list '* (sael->aa x) (sael->aa y)))
          ((equal op '/) (list '/ (sael->aa x) (sael->aa y)))
          ((equal op '^) (list 'expt (sael->aa x) (sael->aa y)))
          (t 0)))))))
  ...
  (match e
    ...
    (:baaexpr
     (match e
       ((op x y)
        (cond
          ((equal op '+) (list (aa->sael x) '+ (aa->sael y)))
          ((equal op '-) (list (aa->sael x) '- (aa->sael y)))
          ((equal op '*) (list (aa->sael x) '* (aa->sael y)))
          ((equal op '/) (list (aa->sael x) '/ (aa->sael y)))
          ((equal op 'expt) (list (aa->sael x) '^ (aa->sael y)))
          (t 0)))))))
       ...
       ((op x y)
        (let ((vx (aaeval x a))
          ...
          (cond
            ...
            ((equal op '-) (- vx vy))
            ((equal op '*) (* vx vy))
            ((equal op '/) (if (equal vy 0) 0 (/ vx vy)))
            ((equal op 'expt)
             (cond
               ((or (equal vy 0) (not (integerp vy))) 1)
               ((equal vx 0) 0)
               (t (expt vx vy))))
            (t 0)))))))) ;unreachable here
...
;; sael succeeds \implies aael preserves value
(property (e :saexpr a :assignment)
  (let ((sv (saeval e a)))
    (=> (rat-okp sv)
        (== sv (aaeval (sael->aa e) a)))))

;; aael succeeds implies sael preserves value
...
;;AI disclosure
;;I used ChatGPT for learning acl2s: mostly asking doubts about LISP. For this particular assignment, I used it to ask the following:
;;1. Error for modeling-set-parms - and identified there is an error with no. of args in the question
;;2. Looked up a measure function I needed - acl2-count
;; So maybe less than 5%  help? Not sure how to quantify that.
;; Note: acl2 treats 0^0 as 1 so acc. to hw rules i have not added error for that.
